...
func (c *OpenAIClient) toWire(req Request) (oaRequest, map[string]json.RawMessage) {
	...
	for _, m := range req.Messages {
		out.Messages = append(out.Messages, c.wireMessage(m))
	}
...
// wireMessage folds one neutral message into its wire shape: role, content,
// the tool calls it requested or answers, and - on assistant turns - the
// dialect's reasoning carrier.
//
// CONTRACT: the echo is UNCONDITIONAL, and byte-verbatim. Every dialect that
// returns reasoning demands it back on every subsequent request of a tool loop
// (DeepSeek and Kimi answer a missing echo with an error; GLM documents it as
// required-unmodified; OpenRouter requires the block sequence to match "the
// outputs generated by the model during the original request" - research
// §matrix "Echo-back in tool loop"). The carrier is passed straight through: it
// is never decoded, re-encoded, reordered or truncated here.
//
// Only assistant messages carry it. A carrier on any other role is a bug
// upstream, and echoing it would hand a strict provider an unknown field on a
// message shape that never has one. A payload captured from a key this dialect
// does not write back is dropped here, not moved to another key
// (echoesReasoningFrom).
func (c *OpenAIClient) wireMessage(m Message) oaMessage {
