...
reports it.** The endpoint decides on its own what to keep, so
`provider.prompt_cache` is a config error on every dialect but one (it
configures the anthropic wire's markers, and the exception below). What amele
does is record the hit: `prompt_tokens_details.cached_tokens` on the OpenAI
spelling and DeepSeek's top-level `prompt_cache_hit_tokens` both land in the
session log's `cache_read_tokens`, and a wire that sends neither logs nothing -
which means "not reported", never "not cached". The exception is `dialect:
openrouter`, where the gateway documents a request-side switch of its own -
see [OpenRouter](#openrouter).
...
sends OpenAI-shaped messages and places none of the three markers above.
What it can do is ask the gateway for its own automatic caching with
`provider.prompt_cache: true` - one body-root `cache_control` key, opt-in on
that dialect - described under [OpenRouter](#openrouter).
...
Prompt caching here is the gateway's to place, and there is one switch for
asking it. An Anthropic model served through OpenRouter caches nothing unless
the request says so; OpenRouter's documented way to say so for a whole
conversation is a single top-level `cache_control` field, which it calls
automatic caching: the gateway puts one breakpoint at the last cacheable block
and moves it forward as the conversation grows - the same moving mark the
anthropic wire's third breakpoint is, without amele touching the message
content. `provider.prompt_cache: true` sends it:
...
provider:
  ...
  base_url: https://openrouter.ai/api/v1
  api_key: ${OPENROUTER_API_KEY}
  prompt_cache: true              # adds "cache_control": {"type": "ephemeral"} at the body root
```

It is **opt-in on this dialect** (the anthropic wire's default is on): the
field is documented by OpenRouter for the Anthropic, Vertex, Azure and
Bedrock providers behind it, but amele has not yet observed it in a live
response (#17), so an existing openrouter config keeps sending exactly the
bytes it always did until you ask. `false` and an omitted key are the same
request. What the gateway then caches, and what it charges, follows
